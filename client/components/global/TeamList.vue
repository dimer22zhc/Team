      ...
      <div v-show="show">
        <div class="my-1" v-for="channel in channels" v-if="!channel.isFavorited">
          <router-link v-if="channel.team_id === team.id" :tag="'div'" :to="$route.params.thread_id ?
                  '/client/home/'+team.id+'/'+channel.id+'/thread/'+$route.params.thread_id :
                  { name:'channel',params:{channel_id:channel.id, team_id:team.id}} "  class="bg-teal-dark flex items-center hover:opacity-100 cursor-pointer  rounded-l pl-8">
